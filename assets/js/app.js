...
const state = {
  ...
  designMode: 'custom', // 'custom' = עיצוב אישי (ברירת מחדל) | 'gallery' = מתוך הגלריה
  designNote: '',       // התיאור שהלקוחה כותבת במילים שלה
  designPhoto: null,    // תמונת השראה שצירפה { name, src }
...
/* -------------------------------------------------- בחירת העיצוב לתור */
/* ברירת המחדל היא "עיצוב אישי": הלקוחה מתארת במילים שלה ויכולה לצרף תמונה.
   מי שבחרה עבודה מהגלריה — עוברת אוטומטית למצב "מהגלריה" ותמיד יכולה להחליף. */

function designSummaryText() {
  if (state.designMode === 'gallery' && state.design) {
    const w = state.design;
    return `${w.title} (${w.shape}, ${w.lengthHe})`;
  }
  const parts = [];
  if (state.designNote) parts.push(state.designNote);
  if (state.designPhoto) parts.push('(מצרפת תמונת השראה בצ’אט)');
  return parts.length ? parts.join(' ') : 'נחליט יחד בטיפול';
}

function renderDesignChoice() {
  const box = $('#designChoice');
  const custom = state.designMode !== 'gallery';

  box.innerHTML = `
    <div class="dc-head">
      <span class="dc-label">העיצוב שלך</span>
      <div class="dc-tabs" role="tablist" aria-label="בחירת סוג העיצוב">
        <button class="dc-tab ${custom ? 'is-on' : ''}" type="button" role="tab"
                aria-selected="${custom}" data-mode="custom">עיצוב אישי</button>
        <button class="dc-tab ${!custom ? 'is-on' : ''}" type="button" role="tab"
                aria-selected="${!custom}" data-mode="gallery">מתוך הגלריה</button>
      </div>
    </div>

    ${custom ? `
      <div class="dc-panel">
        <label class="dc-field" for="designNote">
          ספרי לי מה בא לך — במילים שלך
          <textarea id="designNote" rows="2"
            placeholder="למשל: משהו נקי בגוון ניוד, קצר, בלי ציורים. או פשוט — נחליט יחד בטיפול 🙂">${esc(state.designNote)}</textarea>
        </label>

        <div class="dc-upload">
          <input type="file" id="designPhoto" accept="image/*" hidden>
          <button class="btn btn-outline btn-sm" type="button" id="designPhotoBtn">
            ${state.designPhoto ? 'החלפת התמונה' : 'צירוף תמונת השראה'}
          </button>
          ${state.designPhoto ? `
            <span class="dc-thumb">
              <img src="${state.designPhoto.src}" alt="תמונת ההשראה שצירפת">
              <button class="link-btn" type="button" id="designPhotoClear">הסרה</button>
            </span>` : '<span class="dc-hint">צילום מסך מאינסטגרם? אפשר לצרף כאן</span>'}
        </div>
      </div>` : `
      <div class="dc-panel">
        ${state.design ? `
          <div class="picked-design">
            <img src="${state.design.img}" width="900" height="1200" alt="${esc(state.design.alt)}">
            <span class="pd-text">
              <span class="pd-label">בחרת מהגלריה</span>
              <strong>${esc(state.design.title)}</strong>
              <span class="pd-meta">${esc(state.design.shape)} · ${esc(state.design.lengthHe)}</span>
            </span>
            <button class="link-btn" type="button" data-open-gallery>החלפה</button>
          </div>` : `
          <div class="dc-empty">
            <p>עוד לא בחרת עבודה מהגלריה.</p>
            <button class="btn btn-outline btn-sm" type="button" data-open-gallery>פתיחת הגלריה</button>
          </div>`}
      </div>`}
  `;
  ...
  if (note) {
    note.addEventListener('input', () => { state.designNote = note.value; });
  }

  const fileInput = $('#designPhoto');
  if (fileInput) {
    $('#designPhotoBtn').addEventListener('click', () => fileInput.click());
    fileInput.addEventListener('change', () => {
      const file = fileInput.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = () => {
        state.designPhoto = { name: file.name, src: reader.result };
        renderDesignChoice();
      };
      reader.readAsDataURL(file);
    });
    const clear = $('#designPhotoClear');
  ...
  $$('.dc-tab', box).forEach((tab) => tab.addEventListener('click', () => {
    state.designMode = tab.dataset.mode;
    renderDesignChoice();
  }));
}
...
function renderBooking() {
  ...
  if (state.step === 5) {
    ...
    const waText = [
      ...
      `העיצוב שאני רוצה: ${designSummaryText()}`,
    ...
    body.innerHTML = `
      <div class="confirm">
        ...
        <dl class="summary">
          ...
          <div><dt>העיצוב</dt><dd>${esc(designSummaryText())}</dd></div>
        ...
        ${state.designMode === 'gallery' && state.design ? `
        ...
        ${state.designMode !== 'gallery' && state.designPhoto ? `
        <div class="picked-design confirm-design">
          <img src="${state.designPhoto.src}" alt="תמונת ההשראה שצירפת">
          <span class="pd-text">
            <span class="pd-label">תמונת ההשראה שצירפת</span>
            <strong>${esc(state.designNote || 'עיצוב אישי')}</strong>
          </span>
        </div>` : ''}
